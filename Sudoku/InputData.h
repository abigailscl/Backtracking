...
public:
    ...
    string positiveInteger(string, int min, int max);
...
string InputData<int>::positiveInteger(string message, int min, int max){
 	cout << message;
 	cin  >> value;
 	while (check.positiveInteger(value, min, max)) {
 		cout << "\nIngreso erroneo, vuelva a ingresar: ";
 		cin >> value;
	}
	return value;
